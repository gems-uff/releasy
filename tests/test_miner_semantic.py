...
class describe_release_miner:
    ...
    def it_mine_main_release_commits(self):
        ...
        assert mreleases['0.9.0'].commits.ids == set(['1', '0'])
        assert mreleases['1.0.0'].commits.ids == set(['3', '2'])
        assert mreleases['1.1.0'].commits.ids == set(['6', '5'])
        assert mreleases['2.0.0'].commits.ids \
            == set(['15', '14', '12', '11', '10', '9', '8']) 

    def it_mine_patches_commits(self):
        patches = self.project.patches
        repo = self.project.repository
        assert patches['1.0.2'].commits.ids == set(['13'])
        assert patches['1.1.1'].commits.ids == set(['7', '4'])
        assert patches['2.0.1'].commits.ids == set(['14'])
        assert patches['2.1.1'].commits.ids == \
            set(['20', '19', '17', '18', '16'])
    ...
    def it_mine_base_mreleases(self):
        ...
        assert mreleases['1.1.0'].base_mreleases.names == set(['1.0.0'])
        assert mreleases['2.0.0'].base_mreleases.names == set(['1.0.0', '1.1.0'])
    ...
    def it_mine_main_base_mrelease(self):
        ...
        assert mreleases['1.1.0'].base_mrelease.name == '1.0.0'

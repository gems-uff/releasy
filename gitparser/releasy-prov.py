...
def release_prov(release):
    ...
    for issue in release.issues:
        print("  entity(%d, [Timestamp=%s])" % (issue.id, issue.created.strftime('%Y-%m-%dT%H:%M:%S')))
        print("  used(%s-%s,%d,-)" % ('implement', release.name, issue.id))

        print("  activity(%s-%d, [Label=%s, Timestamp=%s])" % ("implement", issue.id, "implement", issue.created.strftime('%Y-%m-%dT%H:%M:%S')))
